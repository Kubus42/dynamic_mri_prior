...
figure; 
subplot(121); imagesc(abs(p_true)); title('"True" subgradient prior (abs)'); axis image; 
subplot(122); imagesc(abs(p_art)); title('Artificial subgradient prior (abs)'); 
...
w{3} = 0.2 * ones(numel(set{3}),1);
w{4} = 0.2 * ones(numel(set{4}),1);
w{5} = 0.3 * ones(numel(set{5}),1);
w{6} = 0.2 * ones(numel(set{6}),1);
...
% Data fidelity:
alpha{1} = 5 * ones(numel(set{1}),1);
alpha{2} = 5 * ones(numel(set{2}),1);
alpha{3} = 5 * ones(numel(set{3}),1);
alpha{4} = 5 * ones(numel(set{4}),1);
alpha{5} = 5 * ones(numel(set{5}),1);
alpha{6} = 5 * ones(numel(set{6}),1);

%% Run the reconstruction
x0 = zeros(size(u0_clean));
for i = 1:1 %numel(set)    
    [u{i},hist_u{i}] = jr_fmri(f(set{i}), alpha{i}, gamma{i}, w{i}, K(set{i},:), p_art,'param.pdtol',1e-03, 'show',false,'param.niter',5000,'x0',x0);
    x0 = u{i}{10};
end

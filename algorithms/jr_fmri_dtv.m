function [x, hist] = jr_fmri_dtv(data, alpha, gamma, w, op, v, varargin) 

% Computes a complex-valued minimizer x = [x1, ..., xn] of 
%
%   sum_{i=1}^n alpha_i/2 * |B_i x_i - g_i|_C^2 + w_i * |\grad x_i |_1
% + (1-w_i) * dTV(x_i,AP) + gamma_i/2 * |x_{i+1} - x_i|^2
%
...
d_prox3 = @(y, reg) prox_dl1(y,v,reg); %dTV
...
while (it <= param.niter && ~stop_pdres)
    ...
    for i = 1:n
        ...
        dual_3 = y3{i} + sigma3 .* G_x_bar; 


        y1{i} = d_prox1(dual_1,data{i},alpha(i),sigma1); % L2-data term
        y2{i} = d_prox2(dual_2,w(i)); % TV
        y3{i} = dual_3 - sigma3 * d_prox3(dual_3/sigma3,(1-w(i))/sigma3); % dTV
        ...
        primal = x{i} - tau .* (Bt_y1{i} + Gt_y2{i} + Gt_y3{i});
        ...
        elseif (i == 1 && n > 1)
            if exist('x0','var') 
                ...
                x{i} = p_prox(primal,x{i+1},x{i+1},0,gamma(i),tau);
            end
    ...
    if (mod(it, param.int) == 0)
        ...
        for j = 1:n
            % primal
            pr = norm(vec((x_old{j} - x{j})./tau - (Bt_y1_old{j} + Gt_y2_old{j} + Gt_y3_old{j}) + (Bt_y1{j} + Gt_y2{j} + Gt_y3{j})),1) / numel(x{j});
            % dual
            ...
            dr3 = norm(vec((y3_old{j} - y3{j})./sigma3 - G_x_old{j} + G_x{j}),1) / numel(y3{j});
            % residual
            pdres = pdres +  pr + dr1 + dr2 + dr3;  
